...
const char * const BITCOIN_CONF_FILENAME = "ava.conf";
const char * const BITCOIN_PID_FILENAME = "avad.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "ava" is a composite category enabling all Ava-related debug output
            if(ptrCategory->count(string("ava"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\AvaCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\AvaCore
    // Mac: ~/Library/Application Support/AvaCore
    // Unix: ~/.avacore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AvaCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/AvaCore";
#else
    // Unix
    return pathRet / ".avacore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty ava.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override ava.conf
